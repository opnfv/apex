...
if ! grep "$(cat ~/.ssh/id_rsa.pub)" /home/stack/.ssh/authorized_keys; then
    if [ -n /home/stack/.ssh/ ]; then
        sudo mkdir /home/stack/.ssh
	sudo chown stack:stack /home/stack/.ssh
	sudo chmod 700 /home/stack/.ssh
    fi
    sudo cat ~/.ssh/id_rsa.pub >> /home/stack/.ssh/authorized_keys
...
# it mush be stopped to make a copy of its disk image
ssh -T -o "StrictHostKeyChecking no" stack@localhost <<EOI
