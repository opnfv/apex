...
#####  Add OVS DPDK Tools #####
###############################
# michchap: I don't know if these are needed, the puppet is doing a source build+install so may
...
popd > /dev/null
LIBGUESTFS_BACKEND=direct virt-customize --run-command "rm -rf /usr/share/apex-dpdk" \
                                         --run-command "mkdir -p /usr/share/apex-dpdk" \
...
# Upload the dpdk + ovs-dpdk rpms, but don't install it since it will
# replace OVS. The install should be done at deploy time when we
# know if the option is set
guestfs_string=''
