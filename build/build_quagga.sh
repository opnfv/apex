...
install_quagga_build_deps() {
  sudo yum -y install automake bison flex libtool make readline-devel \
               ...
               wget git gcc-c++ openssl-devel boost-devel gtest zeromq-4.1.4 zeromq-devel-4.1.4
...
# Build Quagga
build_quagga(){
  # Quagga needs to have its dependencies built previously installed
...
# Build ZPRC
build_zrpc(){
  # Zrpcd needs to have its dependencies built previously installed
  sudo yum -y install $rpmbuild/RPMS/x86_64/*.rpm
  ...
  make dist
  cat > $rpmbuild/SOURCES/zrpcd.conf <<EOF
hostname bgpd
password sdncbgpc
