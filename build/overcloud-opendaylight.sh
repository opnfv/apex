...
LIBGUESTFS_BACKEND=direct virt-customize \
    ...
    --run-command "sed -i '/  \$retry_interval/c\\  \$retry_interval    = 300,' /etc/puppet/modules/neutron/manifests/plugins/ovs/opendaylight.pp" \
    --run-command "sed -i '/  \$retry_count/c\\  \$retry_count       = 4,' /etc/puppet/modules/neutron/manifests/plugins/ovs/opendaylight.pp" \
