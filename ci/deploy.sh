...
parse_network_settings() {
  local output
  if output=$(python3.4 -B $CONFIG/lib/python/apex-python-utils.py parse-net-settings -n $NETSETS -i $net_isolation_enabled); then
...
function configure_undercloud {
  ...
  if [[ "$net_isolation_enabled" == "TRUE" ]]; then
    ...
    if ! net_env_template=$(python3.4 -B $CONFIG/lib/python/apex-python-utils.py netenv-template -t $CONFIG/network-environment.yaml --network-settings "$network_settings" -n "$enabled_network_list"); then
      echo -e "${red}ERROR: Failed to generate network environment template ${reset}"
      exit 1
    fi
