...
parse_network_settings() {
  if local output=$(python3.4 -B $CONFIG/lib/python/apex-python-utils.py parse-net-settings -f $NETSETS -i $net_isolation_enabled); then
...
function configure_undercloud {
  ...
  if [[ "$net_isolation_enabled" == "TRUE" ]]; then
    ...
    if ! controller_nic_template=$(python3.4 -B $CONFIG/lib/python/apex-python-utils.py nic_template -t $CONFIG/nics-controller.yaml.jinja2 -n "$enabled_network_list" -e $ext_net_type -af $ip_addr_family); then
      echo -e "${red}ERROR: Failed to generate controller NIC heat template ${reset}"
      exit 1
    fi

    if ! compute_nic_template=$(python3.4 -B $CONFIG/lib/python/apex-python-utils.py nic_template -t $CONFIG/nics-compute.yaml.jinja2 -n "$enabled_network_list" -e $ext_net_type -af $ip_addr_family); then
