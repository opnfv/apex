...
function setup_instack_vm {
  if ! virsh list --all | grep instack > /dev/null; then
      ...
      instack_size=$(LIBGUESTFS_BACKEND=direct virt-filesystems --long -h --all -a /var/lib/libvirt/images/instack.qcow2 |grep device | grep -Eo "[0-9\.]+G" | sed -n 's/\([0-9][0-9]*\).*/\1/p')
      if [ "$instack_size" -lt 30 ]; then
        qemu-img resize /var/lib/libvirt/images/instack.qcow2 +25G
	LIBGUESTFS_BACKEND=direct virt-resize --expand /dev/sda1 $RESOURCES/instack.qcow2 /var/lib/libvirt/images/instack.qcow2
        new_size=$(LIBGUESTFS_BACKEND=direct virt-filesystems --long -h --all -a stack/instack.qcow2 |grep filesystem | grep -Eo "[0-9\.]+G" | sed -n 's/\([0-9][0-9]*\).*/\1/p')
