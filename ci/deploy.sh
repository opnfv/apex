...
if [ \$? -ne 0 ]; then echo "Failed to generate network-environment.yaml"; exit 1; fi
mkdir nics/
...
EOF
if [ \$? -ne 0 ]; then echo "Failed to generate nics/controller.yaml"; exit 1; fi
...
EOF
if [ \$? -ne 0 ]; then echo "Failed to generate nics/compute.yaml"; exit 1; fi
EOI
    if [ $? -ne 0 ]; then
        exit 1
    fi
