...
parse_network_settings() {
  ...
  local public_network_optional_settings="floating_ip_range gateway"
...
for Auto-detection${reset}"
    ...
    if [ -n $(eval echo \${${network}_optional_settings}) ]; then
      network_specific_settings=\${${enabled_network}_optional_settings}
...
function setup_instack_vm {
  ...
  if [[ "$net_isolation_enabled" == "TRUE" ]]; then
    virsh attach-interface --domain instack --type network --source public_network --model rtl8139 --config --live
    sleep 1
    # TODO add instack public ip
...
if [[ "$net_isolation_enabled" == "TRUE" ]];
  ...
  sed -i '/ControlPlaneSubnetCidr/c\\  ControlPlaneSubnetCidr: "${admin_network_cidr##*/}"' network-environment.yaml
  sed -i '/ControlPlaneDefaultRoute/c\\  ControlPlaneDefaultRoute: ${admin_network_provisioner_ip}' network-environment.yaml
  sed -i '/ExternalNetCidr/c\\  ExternalNetCidr: ${public_network_cidr}' network-environment.yaml
  sed -i '/ExternalAllocationPools/c\\  ExternalAllocationPools: [{'start': '${public_network_usable_ip_range%%,*}', 'end': '${public_network_usable_ip_range##,*}'}]' network-environment.yaml
  sed -i '/ExternalInterfaceDefaultRoute/c\\  ExternalInterfaceDefaultRoute: ${public_network_gateway}' network-environment.yaml
fi

openstack undercloud install &> apex-undercloud-install.log
EOI

#TODO Move network-environment config to a separate function and copy file to instack
