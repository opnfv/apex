...
function configure_undercloud {
  ...
  if [[ "$net_isolation_enabled" == "TRUE" ]]; then
    ...
    if ! netenv_template=$(python3.4 -B $LIB/python/apex-python-utils.py netenv-template -t $CONFIG/network-environment.yaml.jinja2 --network-settings $NETSETS -n "$enabled_network_list" -i $net_isolation_enabled); then
      echo -e "${red}ERROR: Failed to generate network environment heat template ${reset}"
      exit 1
    fi
