...
if [ -f overcloud-full-numa.qcow2 ]; then
  ...
  KERNEL=\$(glance image-show overcloud-full | grep 'kernel_id' | cut -d '|' -f 3 | tr -d '[[:space]]')
  RAMDISK=\$(glance image-show overcloud-full | grep 'ramdisk_id' | cut -d '|' -f 3 | tr -d '[[:space]]')
