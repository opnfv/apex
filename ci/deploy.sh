...
echo -e $instack_env_output > $CONFIG/instackenv_tmp.json

INSTACKENV=$CONFIG/instackenv_tmp.json
}
...
display_usage() {
  ...
  echo -e "   -d|--deploy_settings : Full path to deploy settings yaml file. Optional.  Defaults to null \n"
  echo -e "   -i|--inventory : Full path to inventory yaml file. Optional. Defaults virtual setup \n"
  ...
  echo -e "   --single_flat_network : disable Network Isolation and use a single flat network for the underlay network."
}
...
main() {
  ...
  if [ ! -z DEPLOY_SETTINGS_FILE ]; then
    parse_deploy_settings
  fi
  setup_instack_vm
  if [ $virtual == "TRUE" ]; then
