...
function configure_deps {
  ...
  for network in ${enabled_network_list}; do
    ovs-vsctl list-br | grep ${network} > /dev/null || ovs-vsctl add-br ${NET_MAP['network']}
    virsh net-list --all | grep ${network} > /dev/null || virsh net-create $CONFIG/${NET_MAP['network']}-net.xml
    virsh net-list | grep -E "${network}\s+active" > /dev/null || virsh net-start ${NET_MAP['network']}
  done
