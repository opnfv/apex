...
function configure_deps {
  ...
  echo "Creating OVS Bridges: $enabled_ovs_bridges"
  /usr/libexec/openstack-tripleo/setup-network '' -b "$enabled_ovs_bridges"
  ...
  virsh pool-list --all | grep default > /dev/null || virsh pool-define-as --name default dir --target /var/lib/images
...
function setup_instack_vm {
  if ! virsh list --all | grep instack > /dev/null; then
      ovs_bridges="default ${NET_MAP["admin_network"]}"
      if [[ $enabled_network_list =~ "public_network" ]]; then
        ovs_bridges+=" ${NET_MAP['public_network']}"
      fi
      define_virtualmachine instack hd 30 "$ovs_bridges"
...
EOF
  ...
  for i in $(seq 0 $vm_index); do
    if ! virsh list --all | grep baremetal${i} > /dev/null; then
      define_virtualmachine baremetal${i} network 41 ${NET_MAP['admin_network']}
      for n in private_network public_network storage_network; do
        if [[ $enabled_network_list =~ $n ]]; then
          virsh attach-interface --domain baremetal${i} --type network --source ${NET_MAP[$n]} --model rtl8139 --config
        fi
      done
    else
