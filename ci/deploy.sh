...
function configure_undercloud {
  ...
  if [[ "$net_isolation_enabled" == "TRUE" ]]; then
    ...
    net_env=$(python3.4 -B $CONFIG/lib/python/apex-python-utils.py netenv-template -d $CONFIG -f network-environment.yaml --network-settings "$network_settings" -n "$enabled_network_list")
    ctl_nic=$(python3.4 -B $CONFIG/lib/python/apex-python-utils.py nic-template -t $CONFIG/nics-controller.yaml.jinja2 -n "$enabled_network_list" -e $ext_net_type -af $ip_addr_family)
    cmp_nic=$(python3.4 -B $CONFIG/lib/python/apex-python-utils.py nic-template -t $CONFIG/nics-compute.yaml.jinja2 -n "$enabled_network_list" -e $ext_net_type -af $ip_addr_family)
    ssh -T ${SSH_OPTIONS[@]} "stack@$UNDERCLOUD" << EOI
set -e
set -x
