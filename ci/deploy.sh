...
function configure_deps {
  ...
  for network in ${OPNFV_NETWORK_TYPES}; do
    echo "${blue}INFO: Creating Virsh Network: $network & OVS Bridge: ${NET_MAP[$network]}${blue}"
...
EOF
    if ! (virsh net-list --all | grep $network > /dev/null); then
        echo "${red}ERROR: unable to create network: ${network}${red}"
        exit 1;
    fi
...
function define_vm () {
  ...
  if virsh vol-list default | grep ${1}.qcow2 2>&1> /dev/null; then
    echo "Volume ${1}.qcow2 exists. Deleting Exiting Volume"
    virsh vol-delete ${1}.qcow2 default
  fi
  ...
  if [ ! -f /var/lib/libvirt/images/${1}.qcow2 ]; then
      echo "/var/lib/libvirt/images/${1}.qcow2 Not created successfully... Aborting"
      exit 1
  fi
