...
function configure_network_environment {
  ...
  if [[ ! -z "$private_network_enabled" && "$private_network_enabled" == "true" ]]; then
      sed -i 's#^.*Network::InternalApi.*$#  OS::TripleO::Network::InternalApi: '${tht_dir}'/internal_api.yaml#' $1
      sed -i 's#^.*Controller::Ports::InternalApiPort:.*$#  OS::TripleO::Controller::Ports::InternalApiPort: '${tht_dir}'/ports/internal_api.yaml#' $1
      sed -i 's#^.*Compute::Ports::InternalApiPort:.*$#  OS::TripleO::Compute::Ports::InternalApiPort: '${tht_dir}'/ports/internal_api.yaml#' $1
      sed -i "/InternalApiAllocationPools/c\\  InternalApiAllocationPools: [{'start': '${private_network_usable_ip_range%%,*}', 'end': '${private_network_usable_ip_range##*,}'}]" $1
      sed -i '/InternalApiNetCidr/c\\  InternalApiNetCidr: '${private_network_cidr}'' $1
  else
      sed -i 's#^.*Network::InternalApi.*$#  OS::TripleO::Network::InternalApi: '${tht_dir}'/noop.yaml#' $1
      sed -i 's#^.*Controller::Ports::InternalApiPort:.*$#  OS::TripleO::Controller::Ports::InternalApiPort: '${tht_dir}'/ports/noop.yaml#' $1
      sed -i 's#^.*Compute::Ports::InternalApiPort:.*$#  OS::TripleO::Compute::Ports::InternalApiPort: '${tht_dir}'/ports/noop.yaml#' $1
  fi
