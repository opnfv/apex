...
if [ -n "$CACHE_DEST" ]; then
    ...
    tar --atime-preserve --dereference -caf ${BUILD_BASE}/${CACHE_NAME}.tgz $BUILD_BASE/$CACHE_DIR/* --xform="s:${BUILD_BASE#/}/$CACHE_DIR/::"
    if [ -f "${BUILD_BASE}/${CACHE_NAME}.tgz" ]; then
      mv ${BUILD_BASE}/${CACHE_NAME}.tgz ${CACHE_DEST}/${CACHE_NAME}.tgz
