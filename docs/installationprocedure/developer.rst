...
This section aims to explain a bit more in detail the steps that Apex follows
...
2.  /bin/: it contains the binaries for the commands opnfv-deploy, opnfv-clean
    and opnfv-util.
...
1.  I can see in the output log a post-deployment error messages:
        ...
        Puppet applies some manifests during this phase. If one of these
        processes fail, you could try to see the error and after that, re-run
        puppet to apply that manifest. Log into the controller (see
        ...
        As a comment, there are four "puppet apply" steps at the moment running
        four different manifests.
