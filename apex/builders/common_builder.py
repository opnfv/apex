...
def project_to_docker_service(project, branch):
    """
    Translates OpenStack project to OOO services that are containerized
    :param project: name of OpenStack project
    :param branch: branch of OpenStack THT to check
    :return: List of OOO service names
    """
    # TODO(trozet): note this method of querying for supported services
    # only takes into account default ones.  We need to update this method
    # to check all services.
    url = "https://raw.githubusercontent.com/openstack/tripleo-heat" \
          "-templates/{}/environments/docker.yaml".format(branch)
    docker_resource_reg = utils.parse_yaml(utils.open_webpage(url))
    if 'resource_registry' not in docker_resource_reg:
        raise exc.ApexCommonBuilderException(
            "Unrecognized docker yaml file content {} from url: {}".format(
                docker_resource_reg, url)
        )
    all_docker_services = docker_resource_reg['resource_registry']
