...
def inject_ovs_nsh(image, tmp_dir):
    ...
    virt_ops = [
        ...
        {con.VIRT_RUN_CMD: "yum install -y rpm-build autoconf automake libtool "
         ...
         "kernel-headers kernel-tools rpmdevtools systemd-units"},
        {con.VIRT_RUN_CMD: "cd /root/ && tar xzf {}".format(ovs_file)},
        {con.VIRT_UPLOAD: "{}/build_ovs_nsh:/root/{}".format(tmp_dir, ovs_folder)},
        {con.VIRT_RUN_CMD: "cd /root/{0} && ./build_ovs_nsh.sh && "
