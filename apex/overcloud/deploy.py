...
def create_deploy_cmd(ds, ns, inv, tmp_dir,
    ...
    if ds_opts['ceph']:
        ...
        deploy_options.append(os.path.join(con.THT_DIR,
...
def external_network_cmds(ns):
    ...
    """
    if 'external' in ns.enabled_networks_list:
    ...
    if nic_config['vlan'] == 'native':
        ext_type = 'flat'
    else:
        ext_type = "vlan --provider-segment {}".format(nic_config['vlan'])
...
def create_congress_cmds(overcloud_file):
    drivers = ['nova', 'neutronv2', 'cinder', 'glancev2', 'keystone']
