...
class TestBuildUtils(object):
    ...
    @patch('apex.build_utils.GerritRestAPI')
    @patch('apex.build_utils.git.Repo')
    def test_clone_fork_raises(self, mock_git_repo,
        ...
        args.branch = 'master'
        x = mock_git_repo.return_value
        x.side_effect = git.exc.InvalidGitRepositoryError()
        xx = x.commit.return_value
        xx.message = '{}: {}'.format(self.repo_name, self.change_id)
        y = mock_gerrit_api.return_value
        y.get.side_effect = KeyError()
        # execute & assert
        assert_raises(KeyError, build_utils.clone_fork, args)

    @patch('apex.build_utils.argparse')
    def test_get_parser(self, mock_argparse):
        build_utils.get_parser()

    @patch('apex.build_utils.get_parser')
    def test_main(self, mock_get_parser):
        x = mock_get_parser.parse_args.return_value
        x.debug = True
        x.func = lambda x: None
        build_utils.main()

    @patch('apex.build_utils.get_parser')
    def test_main_again(self, mock_get_parser):
        x = mock_get_parser.parse_args.return_value
        x.debug = False
        build_utils.main()
