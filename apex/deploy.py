    ...
    if args.quickstart:
        ...
        if upstream:
            ...
            utils.fetch_upstream_and_unpack(args.image_dir, upstream_url,
                                            upstream_targets)
            # copy ramdisk, kernel into libvirt dir
            for boot_file in ['initrd', 'vmlinuz']:
                oc_boot_file = "overcloud-full.{}".format(boot_file)
                shutil.copyfile(
                    os.path.join(args.image_dir, oc_boot_file),
                    os.path.join(constants.LIBVIRT_VOLUME_PATH, oc_boot_file)
                )
