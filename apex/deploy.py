...
def main():
    ...
    if args.quickstart:
        ...
        if upstream:
            ...
            if deploy_settings['global_params'].get('patches'):
                for target in ('undercloud', 'overcloud'):
                    if target == 'undercloud':
                        t_image = uc_image
                    else:
                        t_image = sdn_image
                    if target in deploy_settings['global_params']['patches']:
                        patches = deploy_settings['global_params']['patches'][
                            target]
                        if patches:
                            logging.info("Adding patches to {}".format(target))
                            c_builder.add_upstream_patches(patches, t_image,
                                                           APEX_TEMP_DIR,
                                                           branch)
        else:
        ...
        if os_version != 'ocata':
            ...
            net_data = network_data.create_network_data(net_settings,
                                                        net_data_file)
            # remove resource reg from network-environment as it is
            # automatically generated by OOO in pike and later
            if 'resource_registry' in net_env.keys():
                logging.debug('Removing network-env resource registry')
                del net_env['resource_registry']
                utils.dump_yaml(dict(net_env), net_env_target)
        else:
            net_data = False
        ...
        oc_deploy.create_deploy_cmd(deploy_settings, net_settings, inventory,
                                    APEX_TEMP_DIR, args.virtual, opnfv_env,
                                    net_data=net_data)
