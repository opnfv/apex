    ...
    if args.quickstart:
        ...
        if not upstream:
            ...
            if ds_opts['k8s'] is True:
                k8s_virt_commands = list()
                k8s_virt_commands.append({
                    constants.VIRT_RUN_CMD:
                        "rpm -e --nodeps docker docker-client docker-common"})
                virt_utils.virt_customize(k8s_virt_commands,
                                          overcloud_temp_path)
        ...
        if 'external' in net_settings.enabled_network_list:
            ...
            deploy_vars['external_cidr'] = external_cidr
