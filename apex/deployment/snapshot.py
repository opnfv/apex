...
class SnapshotDeployment:
    ...
    def parse_and_create_nodes(self):
        ...
        for node in node_data['servers']:
            logging.debug('Creating node: {}'.format(pprint.pformat(node)))
            node_xml = os.path.join(self.snap_cache_dir,
                                    '{}.xml'.format(node['name']))
            self.oc_nodes.append(
                OvercloudNode(ip=node['address'],
                              ovs_ctrlrs=node['ovs-controller'],
                              ovs_mgrs=node['ovs-managers'],
                              role=node['type'],
