...
def run_ansible(ansible_vars, playbook):
    """
    ...
    ansible_command = ['ansible-playbook', '--become', '-i', "localhost,",
                       '-c', 'local', playbook, '-vvv']
    ...
    if isinstance(ansible_vars, dict) and ansible_vars:
        ...
        ansible_command.append(ansible_vars)

    try:
        subprocess.check_output(ansible_command, stderr=subprocess.STDOUT)
