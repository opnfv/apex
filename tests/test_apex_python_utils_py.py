...
class TestCommonUtils(object):
    ...
    def setup_class(klass):
        ...
        klass.net_sets = '../config/network/network_settings.yaml'
        klass.net_env = '../build/network-environment.yaml'
        klass.deploy_sets = '../config/deploy/deploy_settings.yaml'
    ...
    def test_build_nic_template(self):
        args = self.parser.parse_args(['nic-template',
                                       '-t', '../build/nics-controller.yaml.jinja2',
                                       '-n', 'admin_network' ])
