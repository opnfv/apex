...
if [ "$external_network_ipv6" == "True" ]; then
  ...
  neutron subnet-create --name external-net --project-id \$(openstack project show service | grep id | awk '{ print \$4 }') external --disable-dhcp external --gateway ${external_gateway} --allocation-pool start=${external_floating_ip_range%%,*},end=${external_floating_ip_range##*,} ${external_cidr}
else
  # we re-use the introspection range for floating ips with single admin network
  neutron subnet-create --name external-net --project-id \$(openstack project show service | grep id | awk '{ print \$4 }') external --disable-dhcp external --gateway ${admin_gateway} --allocation-pool start=${admin_introspection_range%%,*},end=${admin_introspection_range##*,} ${admin_cidr}
fi
