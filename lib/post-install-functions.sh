...
if [ "$external_network_ipv6" == "True" ]; then
  ...
  openstack subnet create external-subnet --project service --network external --no-dhcpe --gateway $external_gateway --allocation-pool start=${external_floating_ip_range%%,*},end=${external_floating_ip_range##*,} --subnet-range $external_cidr
else
