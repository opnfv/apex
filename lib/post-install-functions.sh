...
if [ "$external_network_ipv6" == "True" ]; then
  openstack subnet create external-subnet --project service --network external --dhcp False --gateway $external_gateway --allocation-pool start=${external_floating_ip_range%%,*},end=${external_floating_ip_range##*,} --subnet-range $external_cidr --ip-version 6 --ipv6-ra-mode slaac --ipv6-address-mode slaac
elif [[ "$enabled_network_list" =~ "external" ]]; then
  openstack subnet create --project service --network external --dhcp False --gateway $external_gateway --allocation-pool start=${external_floating_ip_range%%,*},end=${external_floating_ip_range##*,} --subnet-range $external_cidr external-subnet
else
  # we re-use the introspection range for floating ips with single admin network
  openstack subnet create --project service --network external --dhcp False --gateway $admin_gateway --allocation-pool start=${admin_introspection_range%%,*},end=${admin_introspection_range##*,} --subnet-range $admin_cidr external-subnet
fi

if [ "${deploy_options_array['gluon']}" == 'True' ]; then
  echo "Creating Gluon dummy network and subnet"
  openstack network create GluonNetwork --shared --provider-network-type vxlan
  openstack subnet create GluonSubnet --no-gateway --dhcp false --network GluonNetwork --subnet-range 0.0.0.0/1
fi
