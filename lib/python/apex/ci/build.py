...
TMP_CACHE = '.cache'
BUILD_ROOT = 'build'
# TODO(trozet): figure out how to install virtualization host group
BUILD_DEPENDENCIES = ['https://www.rdoproject.org/repos/rdo-release.rpm',
                      'python34', 'python34-devel','python34-jinja2',
                      'python34-markupsafe', 'python2-virtualbmc',
                      'libguestfs-tools', 'bsdtar', 'libvirtd',
                      'python2-oslo-config', 'python2-debt-collector',
                      'http://artifacts.opnfv.org/apex/dependencies/python3'
                      '-ipmi-0.3.0-1.noarch.rpm']
...
def create_build_parser():
    ...
    parser.add_argument('-l', '--log-file',
                        default='/var/log/apex/apex_build.log',
    ...
    parser.add_argument('-r', '--release',
                        dest='build_version',
                        help='Version to apply to build artifact label')
...
def unpack_cache(cache_dest, cache_dir=None):
    if cache_dir is None:
        ...
        try:
            ...
            logging.WARN("Cache unpack failed")
            return
...
def build(build_root, version, iso=False, rpms=False):
    utils.check_package_dependencies(BUILD_DEPENDENCIES)
    if iso:
        make_target = 'iso'
    elif rpms:
        make_target = 'rpms'
    else:
        make_target = 'images rpms-check'
    make_args = 'RELEASE={}'.format(version)
    logging.INFO('Building targets: {}'.format(make_target))
    try:
        subprocess.check_call(["make", make_args, "-C", build_root,
                               make_target])
...
def build_cache(cache_source, cache_dir):
    ...
    :return: None
    """
    cache_name = 'apex-cache-{}.tgz'.format(random.randint(0, 1000000))
    ...
    if os.path.isfile(cache_full_path):
        ...
        logging.WARN("Cache file did not build correctly")
    ...
    while len(cache_entries) > 2:
        ...
        if os.path.isfile(cache_full_path):
            try:
                ...
                logging.WARN("Failed to remove cache file: {}".format(
                    cache_full_path))
                break
...
if __name__ == '__main__':
    ...
    cwd = os.getcwd()
    apex_root = os.getcwd().split('/apex')[0] + '/apex'
