  ...
  if [ "${deploy_options_array['sdn_controller']}" == 'ovn' ]; then
    ...
    ssh -T ${SSH_OPTIONS[@]} "stack@$UNDERCLOUD" <<EOI
      LIBGUESTFS_BACKEND=direct virt-customize \
        --run-command "yum downgrade -y /root/ovs27/openvswitch-2.7* /root/ovs27/openvswitch-devel-2.7* /root/ovs27/openvswitch-test-2.7* /root/ovs27/python-openvswitch-2.7*" \
        -a overcloud-full.qcow2
EOI
  fi
