...
EOI
  ...
  ssh -T ${SSH_OPTIONS[@]} "stack@$UNDERCLOUD" <<EOI
    LIBGUESTFS_BACKEND=direct virt-customize --run-command "cd /usr/lib/python2.7/site-packages/ && patch -p1 < neutron_*_dps.patch && \
                                                            openstack-config --set --list /etc/neutron/plugins/ml2/ml2_conf.ini ml2 extension_drivers data_plane_status" \
                                             -a overcloud-full.qcow2
EOI
